<?=form_open('C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=uri_route'.AMP.'method=edit'.AMP.'id='.$rule['id'], '')?>
...
<table class="mainTable">
	...
	<tr class="even">
		...
		<td><input type="text" name="name" id="name" required value ="<? echo $rule['name']; ?>"/></td>		
	</tr>
	<tr class="odd">
		...
		<td>
            ...
            <select name="group_id[]" id="group_id" size="<?=count($member_groups)+1;?>" multiple>
                ...
                if(strstr($rule['group_id'], ','))  $select_groups = explode(',', $rule['group_id']); else $select_groups = array($rule['group_id']);

                if(isset($member_groups) && count($member_groups) > 0)
                    foreach($member_groups as $user)
                    {
                        $selected = (in_array($user['group_id'], $select_groups))?'selected="selected"':'';
                        printf("<option value='%d' $selected>%s</option>", $user['group_id'], $user['group_title']);
                    }
                ?>
            </select>
	...
	<tr class="even">
		...
		<td><input type="text" id="username" name="username" value="<? if (isset($rule['username']) && $rule['username']!=='') echo $rule['username']; ?>"/><div id="member_list"></div></td>		
	</tr>
	<tr class="odd">
		...
		<td><input type="text" name="template_rules" id="template_rules" value="<?php echo $rule['template_rules']; ?>" /></td>
	...
	<tr class="odd">
		...
		<td><input type="text" name="template_replace" id="template_replace" placeholder="/news/news_full_page/$1/$2" value="<?php echo $rule['template_replace']; ?>" /></td>

	</tr>
	<tr class="even">
        <td><label for="replace_uri"><?=$this->lang->line('replace_uri')?></label><div class="subtext"><?=$this->lang->line('replace_uri_help')?></div></td>
		<td>
      <input name="replace_uri" value="y" type="checkbox" <?php if($rule['replace_uri'] == "y") echo "checked='checked'"; ?> >
		</td>		
	</tr>
    ...
    <tr class="odd">
        ...
        <td>
            <input name="enable" value="on" type="checkbox" <?php if($rule['enable'] == "on") echo "checked='checked'"; ?> >
        </td>
    </tr>
    ...
    <tr class="even">
        ...
        <td>
            ...
            <select name="redirect_type" <?php if($rule['redirect'] == "") echo "disabled"; ?> id="redirect_input">
                <option <?php if($rule['redirect'] == 301) echo "selected='selected'"; ?> value="301">permanent (301)</option>
                <option <?php if($rule['redirect'] == 302) echo "selected='selected'"; ?> value="302">temp (302)</option>
                <option <?php if($rule['redirect'] == 303) echo "selected='selected'"; ?> value="303">seeother (303)</option>
                <option <?php if($rule['redirect'] == 410) echo "selected='selected'"; ?> value="410">gone (410)</option>
            </select>
                </span>
        </td>
    </tr>
